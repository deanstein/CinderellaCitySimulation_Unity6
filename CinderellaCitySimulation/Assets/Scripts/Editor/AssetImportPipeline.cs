...
public class AssetImportUpdate : AssetPostprocessor {
    ...
    void SetClipImportSettings(AudioImporter ai)
    {
        ...
        aiSampleSettings.preloadAudioData = true;
    ...
    public static void SetStandardMaterialEmission(string materialFilePath)
    {
        ...
        if (IsHighDefinitionRenderPipelineActive())
        {
            ...
            Color color = mat.HasProperty("_BaseColor")
                ? mat.GetColor("_BaseColor")
                : mat.GetColor("_Color");

            mat.SetColor("_EmissiveColor", color);

            Texture texture = mat.HasProperty("_BaseColorMap")
            ...
            // disable the separate intensity field so the color value is used directly
    ...
    public static void SetCustomMaterialEmissionIntensity(string materialFilePath, float intensity)
    {
        ...
        if (IsHighDefinitionRenderPipelineActive())
        {
            ...
            // bake intensity directly into _EmissiveColor the same way Built-in did,
            // keeping _UseEmissiveIntensity off so the color value drives the result
        ...
        else
        {
            ...
            // the desired intensity value needs to be modified slightly for proper internal consumption
            float adjustedIntensity = intensity - 0.4169F;

            // redefine the color with intensity factored in
            color *= Mathf.Pow(2.0F, adjustedIntensity);
            mat.SetColor("_EmissionColor", color);
        }

        DebugUtils.DebugLog("<b>Set custom emission intensity of " + intensity + " on Material: </b>" + mat);
    }
    ...
    /// <summary>
    /// True when the default render pipeline is HDRP (Built-in Standard shader paths must not be used).
    /// </summary>
    public static bool IsHighDefinitionRenderPipelineActive()
    {
        ...
        System.Type t = pipelineAsset.GetType();
        if (t.Name == "HDRenderPipelineAsset")
            return true;
    ...
    public static void SetMaterialSpecular(string materialFilePath, byte specularValue)
    {
        ...
        float s = specularValue / 255f;
        Color specularLinear = new Color(s, s, s, 1f);
        ...
        {
            DebugUtils.DebugLog("<b>SetMaterialSpecular skipped (Standard Specular shader missing): </b>" + materialFilePath);
            return;
        }
